...
func (context *Context) valueOf(valuer func(interface{}, *TM_EC.Context) interface{}, value interface{}, meta *Meta) interface{} {
	if valuer != nil {
...
func (context *Context) renderForm(value interface{}, sections []*Section) template.HTML {
	...
	context.renderSectoins(value, sections, []string{"ECResource"}, result, "form")
	return template.HTML(result.String())
}

func (context *Context) renderSectoins(value interface{}, sectoins []*Section, prefix []string, writer *bytes.Buffer, kind string) {
	for _, section := range sectoins {
		var rows []struct {
			Length      int
...
func (context *Context) renderFilter(filter *Filter) template.HTML {
	...
	if err != nil {
		result.WriteString(fmt.Sprintf("got error when render filter template for %v(%v):%v", filter.Name))
	}

	return template.HTML(result.String())
}

func (context *Context) renderMeta(meta *Meta, value interface{}, prefix []string, metaType string, writter *bytes.Buffer) {
	var (
		err      error
	...
	var generateNestedRendrSections = func(kind string) func(interface{}, []*Section, int) template.HTML {
		return func(value interface{}, sections []*Section, index int) template.HTML {
			...
			if len(sections) > 0 {
				...
				context.renderSectoins(value, sections, newPrefix, result, kind)
			}
	...
	funcsMap["render_nested_form"] = generateNestedRendrSections("form")
	defer func() {
		if r := recover(); r != nil {
			debug.PrintStack()
			writter.WriteString(fmt.Sprintf("Get error when render template for meta %v(%v):%v", meta.Name, meta.Type, r))
		}
	}()
...
func (context *Context) indexSections(resouces ...*Resource) []*Section {
	res := context.getResource(resouces...)
	return res.allowedSections(res.IndexAttrs(), context, roles.Read)
}

func (context *Context) editSections(resouces ...*Resource) []*Section {
	res := context.getResource(resouces...)
	return res.allowedSections(res.EditAttrs(), context, roles.Read)
}

func (context *Context) newSections(resouces ...*Resource) []*Section {
	res := context.getResource(resouces...)
	return res.allowedSections(res.NewAttrs(), context, roles.Create)
}

func (context *Context) showSections(resouces ...*Resource) []*Section {
	res := context.getResource(resouces...)
...
func (context *Context) Pagination() *PaginationResult {
	...
	if end < pagination.Pages {
		...
		pages = append(pages, Page{Page: pagination.CurrentPage, IsLast: true})
	}

	return &PaginationResult{Pagination: pagination, Pages: pages}
}

//	'patchCurrentURL' is a convinent wrapper for ec/utils.PatchURL
...
//	'joinURL' is a convinent wrapper for ec/utils.PatchURL
...
func (context *Context) pageTitle() template.HTML {
	if context.Action == "search_center" {
		return context.t("ec_admin.search_center.title", "Search Center")
	}

	if context.Resource == nil {
		return context.t("ec_admin.layout.title", "Admin")
	}
	...
	var (
		defaultValue string
		titleKey     = fmt.Sprintf("ec_admin.form.%v.title", context.Action)
		usePlural    bool
	)
...
func (context *Context) FuncMap() template.FuncMap {
	funcMap := template.FuncMap{
		"current_user":         func() TM_EC.CurrentUser { return context.CurrentUser },
