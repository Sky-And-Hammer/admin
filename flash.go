...
func (context *Context) readFlashFromCookie() (flashes []Flash) {
	if cookie, err := context.Request.Cookie("ec_flashes"); err == nil {
...
func (context *Context) writeFlashes() {
	...
	if bytes, err := json.Marshal(flashes); err == nil {
		...
		utils.SetCookie(http.Cookie{Name: "ec_flashes", Value: base64.StdEncoding.EncodeToString(bytes), Path: prefix}, context.Context)
	}
}
