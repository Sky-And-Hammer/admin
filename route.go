...
func (r *Router) Get(path string, handler requestHanlder, config ...RouteConfig) {
	r.routers["GET"] = append(r.routers["GET"], newRouteHandler(path, handler, config...))
}
...
func (admin *Admin) RegisterResourceRouters(res *Resource, modes ...string) {
	...
	for _, mode := range modes {
		...
		if mode == "delete" {
			if !res.Config.Singleton {
				router.Delete(path.Join(prefix, primaryKey), adminController.Delelte, RouteConfig{
		...
		for _, meta := range res.ConvertSectionToMetas(res.EditAttrs()) {
			if meta.FieldStruct != nil && meta.FieldStruct.Relationship != nil && meta.Resource.base != nil {
				if len(meta.Resource.editSections) > 0 {
					admin.RegisterResourceRouters(meta.Resource, "delete")
				}
			}
		}
	}
...
func (admin *Admin) compile() {
	...
	compileECTemplates := cmdLine.Bool("compile-ec-templates", false, "Complile EC templates")
	cmdLine.Parse(os.Args[1:])
	if *compileECTemplates {
		...
		Handler: func(context *Context, middleware *Middleware) {
			...
			if request.Method != "GET" {
				if browserUserAgentRegexp.MatchString(request.UserAgent()) {
					...
					context.Writer.Write([]byte("Could not authorize you becase 'CSRF detected"))
					return
				}
			}
			middleware.Next(context)
	...
	router.Use(&Middleware{
		...
		Handler: func(context *Context, middleware *Middleware) {
			request := context.Request
			relaticePath := "/" + strings.Trim(strings.TrimSuffix(strings.TrimPrefix(request.URL.Path, router.Prefix), path.Ext(request.URL.Path)), "/")
			handlers := router.routers[strings.ToUpper(request.Method)]
			for _, handler := range handlers {
				if params, ok := handler.try(relaticePath); ok && handler.HasPermission(context.Context) {
					if len(params) > 0 {
...
func (admin *Admin) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	var relatiePath = strings.TrimPrefix(req.URL.Path, admin.router.Prefix)
	...
	if regexp.MustCompile("^/assets/.*$").MatchString(relatiePath) && strings.ToUpper(context.Request.Method) == "GET" {
	...
	var currentUser TM_EC.CurrentUser
	if admin.Auth != nil {
		...
		context.SetDB(context.GetDB().Set("ec:current_user", context.CurrentUser))
	}
