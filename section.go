...
func (res *Resource) generateSections(values ...interface{}) []*Section {
	...
	for i := len(values) - 1; i >= 0; i-- {
		...
		if section, ok := value.(*Section); ok {
			...
			utils.ExitWithMsg(fmt.Sprintf("EC Resource : attributes should be Section or String, but it is %+v", value))
		}
	}
...
func (res *Resource) ConvertSectionToMetas(secitons []*Section) []*Meta {
	var metas []*Meta
	for _, seciton := range secitons {
		for _, row := range seciton.Rows {
			for _, col := range row {
