...
func getField(fileds []*gorm.StructField, name string) (*gorm.StructField, bool) {
...
func (meta *Meta) setBaseResource(base *Resource) {
	...
	res.FindOneHandler = func(value interface{}, metaValues *resource.MetaValues, context *TM_EC.Context) (err error) {
		if metaValues != nil {
	...
	res.FindManyHandler = func(value interface{}, context *TM_EC.Context) error {
		var (
			err       error
			clone     = context.Clone()
			baseValue = base.NewStruct()
	...
	res.SaveHandler = func(value interface{}, context *TM_EC.Context) error {
		var (
			err       error
			clone     = context.Clone()
			baseValue = base.NewStruct()
	...
	res.DeleteHandler = func(value interface{}, context *TM_EC.Context) (err error) {
		clone := context.Clone()
		baseValue := base.NewStruct()
		if primaryKey := res.GetPrimaryValue(context.Request); primaryKey != "" {
			scope := clone.GetDB().NewScope(nil)
			sql := fmt.Sprintf("%v = ?", scope.Quote(res.PrimaryDBName()))
			if err = context.GetDB().First(value, sql, primaryKey).Error; err == nil {
				if err = base.FindOneHandler(baseValue, nil, clone); err == nil {
					base.FindOneHandler(baseValue, nil, clone)
...
func (meta *Meta) updateMeta() {
	...
	if meta.FieldStruct != nil {
		if injector, ok := reflect.New(meta.FieldStruct.Struct.Type).Interface().(resource.ConfigureMetaBeforeInitializeInterface); ok {
			injector.ConfigureECMetaBeforeInitialize(meta)
		}
	}
		...
		if meta.Type == "" {
			...
			if relationship := meta.FieldStruct.Relationship; relationship != nil {
				if (relationship.Kind == "has_one" || relationship.Kind == "has_many") && meta.Meta.Setter == nil && (meta.Type == "select_one" || meta.Type == "select_many") {
					meta.SetSetter(func(resource interface{}, metaValue *resource.MetaValue, context *TM_EC.Context) {
	...
	if meta.Config != nil {
		meta.Config.ConfigureECMeta(meta)
	}
