...
type requestHanlder func(c *Context)

type routeHandler struct {
	Path   string
	Handle requestHanlder
	Config RouteConfig
}

func newRouteHandler(path string, handle requestHanlder, configs ...RouteConfig) routeHandler {
...
func (handler routeHandler) HasPermission(context *TM_EC.Context) bool {
	...
	for i < len(path) {
		...
		case handler.Path[j] == ':':
			...
			if j < len(handler.Path) && handler.Path[j] == '[' {
				...
				if index > 0 {
					match := strings.TrimSuffix(strings.TrimPrefix(handler.Path[j:i+index+1], "["), "]")
